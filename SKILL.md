...
`./ghostwriter <cmd>` — argparse subcommands over `~/.ghostwriter`:

| Command | Does |
|---------|------|
| `status` | Per-contact sent/failed tallies, pause state, dedup + trigger health |
| `add` | Add a Tier 1 contact (`--name --email [--signature --voice-guidelines]`) |
| `edit` | Change a contact's `--email` / `--signature` / `--voice-guidelines` |
| `remove` | Delete a contact and its state file |
| `promote` | Add a sender (e.g. from the digest) as Tier 1 — same as `add` |

Write commands rewrite `config.yaml` via PyYAML (comments not preserved); the
cron pipeline doesn't depend on the CLI. `add`/`promote`/`remove` confirm first
(`--yes` to skip). **Only add addresses you control** — a Tier 1 reply is
auto-sent to the contact's own address.
...
footer points back at `ghostwriter promote`.
